...
public class GithubService(string githubApiToken) {
    ...
    public IObservable<GithubInfo> GetRelatedRepositories(HashSet<string> topics) {
        var observables = topics.Select(topic =>
            Observable.FromAsync(() => FetchReposAsync(topic)).SubscribeOn(TaskPoolScheduler.Default).SelectMany(repos => repos));
        return observables.Merge().ObserveOn(new EventLoopScheduler());
    }
